...
function Table() {
  ...
  const onSubmit = () => {
    if (temp === null || temp === undefined) {
      dispatch(setError({error: "Temperature cannot be empty."}));
      return;
    }
        ...
        <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <div className="shadow border-b border-gray-200 sm:rounded-lg">
            ...
            <label className="relative block items-center text-center m-auto mt-10">
              ...
              <input className="placeholder:italic placeholder:text-slate-400 block bg-white w-25 border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm text-center items-center m-auto mb-10" placeholder="Add a temperature in F..." type="text" name="search" value={temp} onChange={(e) => setTemp(e?.target?.value)}/>
            </label>
            <button class="bg-green-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-10" onClick={onSubmit}>Enter Temp</button>
          </div>
      </div>
    </div>
  )
